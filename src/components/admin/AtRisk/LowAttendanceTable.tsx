...
function pctColor(pct: number) {
  return pct >= 50 ? "text-yellow-600" : "text-red-500";
}
...
export function LowAttendanceTable() {
  ...
  useEffect(() => {
    fetch("/api/admin/at-risk/attendance", { credentials: "include" })
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        <h2 className="font-semibold text-gray-800">Low Attendance</h2>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Present / Total</th>
            ...
            {loading ? (
              ...
              rows.map((row, index) => (
                <tr key={row.studentId} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={`px-6 py-3.5 font-semibold ${pctColor(row.pct)}`}>{row.pct}%</td>
                  <td className="px-6 py-3.5 text-xs text-gray-500">
